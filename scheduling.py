...
class Scheduler:
    ...
    def schedule(self, task_id: Hashable, coroutine: Coroutine) -> None:
        ...
        """
        self._log.debug("Scheduling task #%s...", task_id)
    ...
    def schedule_later(self, delay: float, task_id: Hashable, coroutine: Coroutine) -> None:
        """
    ...
    def cancel(self, task_id: Hashable) -> None:
        ...
        """
        self._log.debug("Cancelling task #%s...", task_id)
    ...
    async def _await_later(self, delay: float, task_id: Hashable, coroutine: Coroutine) -> None:
        """Await ``coroutine`` after ``delay`` seconds."""
        try:
            self._log.debug("Waiting %s seconds before awaiting coroutine for #%s.", delay, task_id)
            await asyncio.sleep(delay)

            # Use asyncio.shield to prevent the coroutine from cancelling itself.
            self._log.debug("Done waiting for #%s; now awaiting the coroutine.", task_id)
    ...
    def _task_done_callback(self, task_id: Hashable, done_task: asyncio.Task) -> None:
        ...
        """
        self._log.debug("Performing done callback for task #%s %s.", task_id, id(done_task))
        ...
        if scheduled_task and done_task is scheduled_task:
            ...
            self._log.debug("Deleting task #%s %s.", task_id, id(done_task))
...
def _log_task_exception(task: asyncio.Task, *, suppressed_exceptions: tuple[type[Exception], ...]) -> None:
    ...
    with contextlib.suppress(asyncio.CancelledError):
        ...
        if exception and not isinstance(exception, suppressed_exceptions):
            ...
            log.error("Error in task %s %s!", task.get_name(), id(task), exc_info=exception)
